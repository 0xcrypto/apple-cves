    ...
    try:
        ...
        async with session.get(NVD_API_URL, params=params) as response:
            if response.status == 403 or response.status == 429:
                print(
                    f"Rate limited by NVD for {cve_id} (Status {response.status}). Waiting longer..."
                )
    ...
    while True:
        ...
        if known_severity:
            ...
            file_path = os.path.join(
                BASE_DIR, known_severity, cve, item["platform"], "advisory.md"
            )
            if os.path.exists(file_path):
                print(
                    f"Skipping {cve} ({item['platform']}) - already exists in {known_severity}"
                )
                queue.task_done()
                continue
...
def update_readme():
    ...
    severity_counts = {
        severity: {tech: 0 for tech in all_technologies}
    ...
    for severity in SEVERITY_MAPPING.values():
        ...
        for cve in cves:
            ...
            for p in found_platforms:
                ...
                severity_counts[severity][p] += 1

    # Build Table
    # Header
    header = "| Severity | " + " | ".join(tech_columns) + " | Other |\n"
            ...
            if "Apple security updates" not in text and "archive" not in text.lower():
                if href not in processed_urls:
                    ...
                    archive_tasks.append(
                        process_advisory(session, href, text, nvd_queue)
                    )
